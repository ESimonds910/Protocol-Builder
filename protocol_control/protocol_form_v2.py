...
class ProtocolForm:
    def __init__(self):
        ...
        # self.standard_details = self.st_details.standard_details()
    ...
    def add_standard_data(self, event):
        ...
        if self.add_standard_clicked:
            ...
            if project == "...":
                ...
                # self.standard_details.children = [ipw.HTML(f"{project}")]
    ...
    def capture_inputs(self, event):
        ...
        # self.data_dict.update(proj_details_dict, plate_details_dict)
        # self.out_display.children = [ipw.HTML(f"{value}") for value in self.data_dict.values()]
        with open('captured_data.json', 'w') as captured_data:
            json.dump(self.data_dict, captured_data, indent=4)
        # display(self.data_dict)
...
class ProjectDetails:
    def __init__(self):
        ...
        # self.db = DbQuery(self.params)
        # temp comment
        # projects = self.db.choose_project()
    ...
    def __init__(self):
        ...
        def get_predilution(pd_check):
            if pd_check:
                ...
                # self.input_display.children += (self.predilution_display,)
            else:
...
class StandardDetails:
    def __init__(self):
        ...
        # self.standard_fold_6 = ipw.IntText(description='Fold #6: ')
        ...
        # self.standard_placeholder = ipw.VBox([self.standard])
        ...
        # def include_standards(standard_check):
        #     if standard_check:
        #         self.standard_placeholder.children = self.standard_info
        #     else:
        #         self.standard_placeholder.children = []
        #
        # ipw.interactive(include_standards, standard_check=self.standard_include)
...
class Outputs:
    ...
    def run_outputs(self, event):
        ...
        # Temp comment
        # self.reagent_display, reagent_details = AssaySolutions(self.inputs, self.output_dict).get_reagent_details()
        ...
        # self.output_dict['assay_db'] = assay_db
...
class StandardData:
    ...
    def calculate_standards(self):
        ...
        # temp comment
        # standard_id, standard_stock_conc = self.get_standard_details()
        # excel standard
        # standard_stock_nm = self.get_excel_standard()
        ...
        standard_details = dict(
            ...
            # standard_transfer_vols=transfer_vols,
            # standard_dbi_pre_vols=dbi_pre_vols,
        )
    ...
    def get_excel_standard(self):
        standard_data = ExcelData().get_standard_data(self.project)
        standard_stock_conc = standard_data["Conc. (nM)"].squeeze()
        return standard_stock_conc

    def get_standard_details(self):
        select_query = f"""
            SELECT standard_id, standard_name, stock_conc_nm, on_hand
            FROM project_standards
            WHERE proj_id = {self.proj_id}
        """
        standard_data = self.db.query_call(select_query)
        standard_stock_conc = float(standard_data[0][2])
        standard_id = int(standard_data[0][0])

        return standard_id, standard_stock_conc

    def setup_form(self):
        well_labels = [f"Well {num}" for num in range(1, 7)]
    ...
    def get_transfer_values(self):
        transfer_vols = ["0"] + [str(round(self.standard_base_wvol / (fold - 1), 0)) for fold in self.all_folds.values()]
        return transfer_vols

    def get_dbi_pre_vols(self):
        dbi_pre_vols = [self.standard_total_vol.value] + [self.standard_base_wvol for repeat in range(0, 5)]
        return dbi_pre_vols
...
class ExcelReagents:
    ...
    def get_reagents(self):
        ...
        # self.calculate_data(reagent_dict)
        return reagent_dict

    def calculate_data(self):
        assay_req = self.output_dict['assay_req']
        reagent_records = [["Reagent", "Assay", "Cat_Num", "Conc. (ug/uL)", "Conc. (nM)", 'Desired conc. (nM)', 'Needed vol (uL)']]

        for reagent in self.reagents:
            conc_ug_ul = reagent[-3]
            desired_conc = reagent[-1]
        ...
        for reagent in self.reagent_dict:
            try:
                ...
                if 'bead' in reagent["Reagent"]:
                    ...
                    reagent_record = [reagent_name, assay_num, cat_num, conc_ug_ul, conc_nm, desired_conc, needed_vol]
        ...
        # return reagent_display, assay_details, assay_db
    ...
    def prepare_reagent_tables(self, reagent_records, assay_req):
        ...
        for assay in assay_list:
            ...
            # assay_db[f"Assay {assay}"] = [[round(assay_req - (sum(
            #     [float(row[-1]) for row in reagent_records[1:] if row[1] == assay]
            # ) / 1000), 3)]]

        # return assay_table_dict, assay_db
            ...
            if in_key == "pd_vols_data":
                ...
                for plate, plate_data in in_value.items():
                    ...
                    for inner_data, inner_list in plate_data.items():
                        data_start_row = start_row
                        # ws.cell(row=start_row - 3, column=start_col).value = plate
    ...
    def fetch_template(self):
        if self.proj_type == "Fermentation":
            location_dict = dict(
                ...
                pd_3_vol="H45",
                pd_4_vol="H46",
                standard_total_vol="F32",
                standard_total_stock="F33",
                standard_total_dbi="F34",
            ...
            location_dict = dict(
                ...
                dil_vol_3="D41",
                dil_vol_4="D42",
                standard_total_vol="F26",
                standard_total_stock="F27",
                standard_total_dbi="F28",
